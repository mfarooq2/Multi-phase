...
@author: Mr.Landis_the_Magnificent 
"""
...
def D_plus(phi_plus, phi):
    return phi_plus - phi

def D_minus(phi, phi_minus):
    return phi - phi_minus
...
for iteration in range(0,100):
    ...
    # Now let's advect the level set field, phi
    # Do the predictor step for level set field advection
    for i in range(0,len(xlist)):
        for j in range(0,len(ylist)):
            if i == len(xlist)-1: 
                D_plus_grid[i,j] = D_plus(phi[0,j] , phi[i,j])
                D_minus_grid[i,j] = D_minus(phi[i,j],phi[i-1,j])
                #D_plus( phi[i+1,j] , phi[i,j])
            elif i == 0:
                D_plus_grid[i,j] = D_plus( phi[i+1,j] , phi[i,j])
                D_minus_grid[i,j] = D_minus(phi[i,j],phi[-1,j])
            else:
                D_plus_grid[i,j] = D_plus( phi[i+1,j] , phi[i,j]) 
                D_minus_grid[i,j] = D_minus(phi[i,j],phi[i-1,j])
