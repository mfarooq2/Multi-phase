...
def p_looper(cell_S_x_us, cell_S_y_vs, cell_cent_pn, cell_cent_pnn):
    epstot=0
    for i in prange(2, Nx1): 
        for j in prange(1,Nx2+1):
            U_s=(cell_cent_rho[i,j]/(dt*(Lx1/Nx1)))*(cell_S_x_us[i+1,j]-cell_S_x_us[i,j]+cell_S_y_vs[i,j+1]-cell_S_y_vs[i,j])
    ...
    return cell_cent_pn, cell_cent_pnn, epstot
@nb.jit(parallel=True)
def unn_looper(cell_S_x_us, cell_cent_pnn, cell_S_x_unn):
    for j in prange(1, Nx2+1):
        for i in prange(1, Nx1+1):
...
while n_iter<=1300:
    ...
    for j in range(1, Nx2+1):
        for i in range(1, Nx1+1):
            ...
            if n_iter==0:
                cell_S_x_us[i,j]=cell_S_x_un[i,j]+dt*(-Adv_x_n(i,j)+(cell_cent_mu[i,j]/cell_cent_rho[i,j])*Dif_x_n(i,j)+(-gradP)/cell_cent_rho[i,j])
            else:
                cell_S_x_us[i,j]=cell_S_x_un[i,j]+dt*(-Adv_x_n(i,j)+(cell_cent_mu[i,j]/cell_cent_rho[i,j])*Dif_x_n(i,j)+(-gradP)/cell_cent_rho[i,j])
    epstot=100.0
    while epstot>1e-3:
        ...
        cell_cent_pn, cell_cent_pnn, epstot=p_looper(cell_S_x_us, cell_S_y_vs, cell_cent_pn, cell_cent_pnn)    
    ...
    for i in range(0, Nx1+2):
        cell_cent_phis[i,0]=cell_cent_phis[i,1]
        cell_cent_phis[i,-1]=cell_cent_phis[i,-2]
    ...
    for j in range(0, Nx2+2):
        ...
        #cell_cent_phi_dn[0,j]=cell_cent_phin[0,j]
        #cell_cent_phi_dn[-1,j]=cell_cent_phin[-1,j]
    ...
    for i in range(0, Nx1+2):
        ...
        #cell_cent_phi_dn[i,0]=cell_cent_phin[i,0]
        #cell_cent_phi_dn[i,-1]=cell_cent_phin[i,-1]
    ...
    while tau<=M*h:
        ...
        for j in range(0, Nx2+2):         
            ...
            cell_cent_phi_ds[-1,j]=cell_cent_phi_ds[1,j]
        for i in range(0, Nx1+2):
            cell_cent_phi_ds[i,0]=cell_cent_phi_ds[i,1]
            cell_cent_phi_ds[i,-1]=cell_cent_phi_ds[i,-2]
        ...
        for j in range(0, Nx2+2):    
            ...
            cell_cent_phi_dnn[-1,j]=cell_cent_phi_dnn[1,j]
        for i in range(0, Nx1+2):   
            cell_cent_phi_dnn[i,0]=cell_cent_phi_dnn[i,1]
            cell_cent_phi_dnn[i,-1]=cell_cent_phi_dnn[i,-2]
        #cell_cent_phi_dnn=phi_dnn_BC_looper(cell_cent_phi_dnn)
        cell_cent_phi_dn=cell_cent_phi_dnn
        tau+=dtau
