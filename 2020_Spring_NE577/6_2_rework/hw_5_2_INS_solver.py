...
def L_phi_n(i,j):
    def phi_xh_n(i,j):
        if 0.5*(cell_S_x_unn[i+1,j]+cell_S_x_unn[i,j])>0:
            ...
            return cell_cent_phin[i+1,j]-0.5*M_sw(D_x_p_n(i+1,j), D_x_p_n(i+1,j))
...
def DDd_pm_y_n(i,j):
    if j==Nx2+1:
        ...
        #cell_cent_phi_dn[i,1]-2*cell_cent_phi_dn[0,j]+cell_cent_phi_dn[i,-1]
...
def Dtda_y_p_n(i,j):
    return Dd_y_p_n(i,j)-0.5*M_sw(DDd_pm_y_n(i,j), DDd_pm_y_n(i,j+1))

def Dtda_y_m_n(i,j):
    return Dd_y_m_n(i,j)+0.5*M_sw(DDd_pm_y_n(i,j), DDd_pm_y_n(i,j-1))
...
def Dtda_y_p_s(i,j):
    return Dd_y_p_s(i,j)-0.5*M_sw(DDd_pm_y_s(i,j), DDd_pm_y_s(i,j+1))

def Dtda_y_m_s(i,j):
    return Dd_y_m_s(i,j)+0.5*M_sw(DDd_pm_y_s(i,j), DDd_pm_y_s(i,j-1))
...
def grad_phi_x(i,j):
    return (cell_cent_phin[i+1,j]-cell_cent_phin[i,j])/(h)

def grad_phi_y(i,j):
    return (cell_cent_phin[i,j+1]-cell_cent_phin[i,j])/(h)
...
def f_st_y(i,j):
    if dfdphi(i,j)!=0:
        return st_coef*kappa(i,j)*dfdphi(i,j)*grad_phi_y(i,j)
    else:
        return 0
...
while n_iter<=1300:
    ...
    for i in range(0, int(M*h/dtau)):
        ...
        for i in range(0, Nx1+2):   
            cell_cent_phi_dnn[i,0]=cell_cent_phi_dn[i,1]
